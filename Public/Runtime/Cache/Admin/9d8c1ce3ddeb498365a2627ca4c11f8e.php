...
function contentNewsListAdd(){
	var url = '<?php echo U('Content/add_news_iframe?catid='.$catid);?>';
...
function contentNewsListEdit(id){
	...
	var url = '<?php echo U('Content/edit_news_iframe?catid='.$catid);?>';
...
function contentNewsListDelete(id){
	...
	$.messager.confirm('提示信息', '确定要删除吗？', function(result){
		...
		$.post('<?php echo U('Content/delete_news?catid='.$catid);?>', {ids: ids}, function(res){
			if(!res.status){
...
function contentNewsListOrder(){
	$.post('<?php echo U('Content/order_news?catid='.$catid);?>', $('.content_gridlist_order_input').serialize(), function(res){
		if(!res.status){
			$.messager.alert('提示信息', res.info, 'error');
		}else{
			$.messager.alert('提示信息', res.info, 'info');
